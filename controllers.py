...
class Payment:
    ...
    def pay(self, billing_cur):
        ...
        r = billing_cur.send_payment_info(self.order.buyer.BoundPayment, self.order.product.seller.BoundPayment, self.order.product.price, order_id)
    ...
    def pay_finish(self, billing_cur):
        return billing_cur.payment_result_success(self.request)

    def pay_cancel(self, billing_cur):
        return billing_cur.payment_result_cancel(self.request)
...
class Check_control:
    ...
    def check(self):   #审核未经审核的商品
        ...
        if self.request.method == 'POST':
            ...
            if state_cur == '0':
                product.state = 0
                Interfaces.Isubmit.submit(self)
                product.save()  
                messages.add_message(self.request, messages.SUCCESS, "通过")
            if state_cur == '1':
                product.state = -2   #-2代表商品审核不通过
                Interfaces.Isubmit.submit(self)
                product.save()
                messages.add_message(self.request, messages.SUCCESS, "不通过")
       
        all_products = models.Product.objects.filter(state=-1).order_by("-id")#显示未审核的商品，-1代表商品还未经审核
