...
@login_required(login_url='/login/')
def pay(request):
    return controllers.Payment_control(request).pay(External.BillingSystem())

# 接收第三方支付平台返回信息，成功场景
@csrf_exempt
def pay_finish(request):
    return External.Pay_result_testpay(request).success()

# 接受第三方支付平台返回信息，非核心场景
@csrf_exempt
def pay_cancel(request):
    return External.Pay_result_testpay(request).cancel()
...
def college_authenticate(request):
    return controllers.College_authenticate_control(request).authenticate(External.Email_sending())
